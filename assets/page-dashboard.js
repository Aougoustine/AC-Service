...
function renderDashboard(){
  ...
  const rdvToday = state.appointments.filter(a=>a.date===today && a.status!=='Annulé' && a.requestType!=='depot');
  const rdvUpcoming = state.appointments.filter(a=>a.date>today && a.status!=='Annulé' && a.status!=='Terminé' && a.requestType!=='depot');
  ...
  main.innerHTML = `
    ...
    <div class="stat-grid">
      ...
      <div class="stat-card ${depositRequestsPending?'warn':''}"><div class="num">${depositRequestsPending}</div><div class="lbl">📥 Demandes de dépôt en attente</div></div>
  ...
  if(!list.length){
    ...
    dashRdv.innerHTML = list.map(a=>`
      <div class="item-row">
        <div class="item-main">
          <div class="title">${a.requestType==='depot'?'📥':'🏠'} ${escapeHtml(a.clientName)} — ${escapeHtml(a.applianceType||'')}</div>
