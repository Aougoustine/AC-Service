...
const RDV_STATUSES = ["À faire","Attente pièces","Terminé","Annulé"];
const INTERV_STATUSES = ["À faire","Attente pièces","Terminé","Annulé"];
// Anciens statuts (avant la simplification à 4 valeurs) → nouveaux statuts.
const LEGACY_INTERV_STATUS_MAP = {
  "Brouillon":"À faire", "En cours":"À faire", "Diagnostiqué":"À faire",
  "Devis envoyé":"À faire", "Pièce commandée":"Attente pièces",
  "En attente de paiement":"Attente pièces", "Facturé":"Terminé"
};
...
function migrateInterventionStatus(i){
  ...
  i.status = LEGACY_INTERV_STATUS_MAP[i.status] || 'À faire';
  return i;
}
// Ramène un appareil rempli avant la simplification (menu déroulant "Résultat de
// l'intervention") vers les 2 cases à cocher indépendantes, sans rien perdre.
// legacyPaymentReceived vient de l'ancien statut global "Paiement effectué" du
// rapport, repris pour le forfait si l'appareil n'a pas encore été migré vers le
// nouveau modèle (forfait + prix pièces, chacun payable indépendamment).
function migrateDeviceResult(d, legacyPaymentReceived){
  if(d.needsPart === undefined) d.needsPart = (d.result === 'Pièce à commander (2nd passage)');
  if(d.irreparable === undefined) d.irreparable = (d.result === 'Irréparable' || d.passageType === 'Irréparable');
  if(d.forfaitPrice === undefined && d.partsPrices === undefined){
    d.forfaitPrice = d.amount || '';
    d.forfaitPaid = !!legacyPaymentReceived;
    d.partsPrices = [];
  }
  if(!d.partsPrices) d.partsPrices = [];
  return d;
}
// Libellé imprimé dans le rapport (PDF) et affiché dans les Dossiers — dérivé des
// 2 cases à cocher, avec repli sur l'ancien champ texte pour les rapports non ré-enregistrés.
// Prix d'un appareil = forfait fixe + somme des prix pièces ajoutés (chacun peut
// être coché "Payé" indépendamment). Partagé entre le rapport (calcul du total) et
// les Dossiers (calcul du reste à payer).
function computeDeviceAmount(d){
  const forfait = Number(d.forfaitPrice)||0;
  const parts = (d.partsPrices||[]).reduce((s,p)=>s+(Number(p.amount)||0), 0);
  return forfait + parts;
}
// Part non payée du prix de cet appareil (forfait si pas coché + chaque prix pièce
// non coché).
function computeDeviceUnpaid(d){
  let unpaid = 0;
  if(!d.forfaitPaid) unpaid += Number(d.forfaitPrice)||0;
  (d.partsPrices||[]).forEach(p=>{ if(!p.paid) unpaid += Number(p.amount)||0; });
  return unpaid;
}
// Reste à payer total d'un rapport, tous appareils confondus.
function interventionUnpaidAmount(iv){
  return (iv.devices||[]).reduce((s,d)=>s+computeDeviceUnpaid(d), 0);
}

function deviceResultLabel(d){
  if(d.irreparable === true) return 'Irréparable';
  if(d.needsPart === true) return 'Pièce à commander';
  if(d.irreparable === undefined && d.needsPart === undefined){
    if(d.result === 'Irréparable' || d.passageType === 'Irréparable') return 'Irréparable';
    if(d.result === 'Pièce à commander (2nd passage)') return 'Pièce à commander';
    if(d.result || d.passageType) return 'Réparé';
  }
  return 'Réparé';
}
// Nettoie les données à chaque chargement depuis le serveur : anciens statuts de
// RDV ("Confirmé"...) ramenés à "À faire", anciens statuts de rapport et anciens
// résultats d'appareil migrés vers les nouveaux formats. Appliqué une seule fois
// ici plutôt que dispersé dans chaque page.
function normalizeLoadedState(state){
  if(!state) return state;
  (state.appointments||[]).forEach(a=>{
    if(!RDV_STATUSES.includes(a.status)) a.status = 'À faire';
  });
...
function startAutoRefresh(onUpdate, intervalMs){
  ...
  setInterval(async ()=>{
    ...
    try{
      ...
      if(!res.ok) return;
      const data = await res.json();
      if(data && data.state) onUpdate(normalizeLoadedState(data.state));
...
function statusBadgeClass(s){
  return {
    "À faire":"badge-nouveau","Nouveau":"badge-nouveau","En cours":"badge-encours",
    "Terminé":"badge-confirme","Annulé":"badge-annule","Brouillon":"badge-nouveau","Attente pièces":"badge-encours",
...
function renderNavbar(active, state){
  ...
  const planningBadge = state.appointments.filter(a=>a.status==='À faire' && a.date>=today && a.date<=horizonEnd && a.requestType!=='depot').length;
...
async function buildReportPdf(i, company){
  ...
  for(let idx=0; idx<devices.length; idx++){
    ...
    reparationFields.push({ label: 'Résultat de l\'intervention', value: deviceResultLabel(d) });
    if(deviceResultLabel(d) === 'Irréparable' && d.insuranceComment){
    ...
    const billingFields = [];
    if(d.forfaitPrice){
      billingFields.push({ label: 'Forfait', value: money(d.forfaitPrice) + (d.forfaitPaid ? ' (payé)' : ' (non payé)') });
    }
    (d.partsPrices||[]).forEach(p=>{
      if(!p.amount) return;
      billingFields.push({ label: p.label || 'Pièce', value: money(p.amount) + (p.paid ? ' (payé)' : ' (non payé)') });
    });
    billingFields.push({ label: 'Montant total appareil', value: money(computeDeviceAmount(d)) });
    drawFramedSection('Facturation', billingFields);
    totalAmount += computeDeviceAmount(d);
  ...
  const unpaidTotal = interventionUnpaidAmount(i);
  field('Paiement', unpaidTotal > 0.001 ? ('Reste à payer : ' + money(unpaidTotal)) : 'Entièrement payé');
  if(i.observations){
    y += 2;
    field('Observations supplémentaires', i.observations);
  }
...
async function sendReportToClient(i, company, includeReviewRequest){
  ...
  const allRepaired = devicesForOutcome.length>0 && devicesForOutcome.every(d=>deviceResultLabel(d)==='Réparé');
  const allIrreparable = devicesForOutcome.length>0 && devicesForOutcome.every(d=>deviceResultLabel(d)==='Irréparable');
