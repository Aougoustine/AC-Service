...
function buildAllDossierGroups(){
  ...
  state.appointments.forEach(a=>{
    if(!['Confirmé','Terminé'].includes(a.status)) return;
...
function drawDossList(){
  ...
  el.innerHTML = groups.map(g=>{
    ...
    const anyUnpaid = hasInterv && g.interv.some(iv=>!iv.paymentReceived);
    const deviceLabel = g.interv[0] ? dossierDeviceSummary(g.interv[0]).title : (g.rdv ? (g.rdv.applianceType||'') : '');
    const summaryTitle = [
      g.dossierNumber ? `<span style="color:var(--cyan);">${escapeHtml(g.dossierNumber)}</span>` : '',
      escapeHtml(g.clientName||'—'),
      deviceLabel ? escapeHtml(deviceLabel) : ''
    ].filter(Boolean).join(' — ');
    const statusPill = hasInterv
      ? `<span class="payment-pill ${anyUnpaid?'payment-pill-unpaid':'payment-pill-paid'}">${anyUnpaid?'❌ Impayé':'✅ Payé'}</span>`
      : (g.rdv ? `<span class="badge ${statusBadgeClass(g.rdv.status)}">${g.rdv.status}</span>` : '');
    ...
    return `
      <div style="display:flex;align-items:flex-start;gap:.6rem;">
        ...
        <summary class="dossier-summary">
          <div class="item-main">
            <div class="title">🗂️ ${summaryTitle}</div>
            <div class="meta">${fmtDate(g.date)}${g.rdv?(' · 📍 '+escapeHtml(g.rdv.address||'—')):''}</div>
          </div>
          ${statusPill}
        </summary>
...
function openReportDetail(id){
  ...
  function renderDeviceBody(idx){
    ...
    const fields = [
      ...
      { label: 'Résultat', value: d.result || d.passageType },
