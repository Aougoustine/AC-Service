...
function renderDepots(){
  ...
  main.innerHTML = `
    ...
    <div class="section-block" id="depRequestsBlock" style="display:none;">
      <h3>📋 Demandes de dépôt en attente</h3>
      <p style="font-size:.85rem;color:var(--text-light);margin-bottom:1rem;">
        Ces demandes ne créent pas de rendez-vous dans Planning — appelle le client pour convenir d'un jour, puis fais son devis et son bon de dépôt directement ici.
      </p>
...
function drawDepRequestsList(){
  ...
  const requests = state.appointments.filter(a=>a.requestType==='depot' && !['Annulé'].includes(a.status) && !depositedRdvIds.has(a.id));

  block.style.display = requests.length ? 'block' : 'none';
  ...
  el.innerHTML = requests.map(a=>{
    ...
    return `
      <div class="item-row" data-id="${a.id}">
        <div class="item-main">
          <div class="title">${a.dossierNumber?`<span style="color:var(--cyan);">${escapeHtml(a.dossierNumber)}</span> — `:''}${escapeHtml(a.clientName||'—')}</div>
...
function openDepositForm(linkedRdv){
    ...
    <div class="modal-overlay" id="depositFormOverlay">
      <div class="modal">
        ...
        <div class="modal-footer">
          <button class="btn btn-ghost" id="depFormCancel">Annuler</button>
          <button class="btn btn-primary" style="width:auto" id="depFormSave">💾 Enregistrer le dépôt</button>
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('depFormCancel').onclick = ()=>{
    ...
    <div class="modal-overlay" id="depositDetailOverlay">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-ghost" id="depDetailCreateReport">📝 Créer le rapport</button>
        </div>
      </div>
    </div>
  `;
