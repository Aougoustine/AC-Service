...
function openClientModal(key){
  ...
  modalRoot.innerHTML = `
    <div class="modal-overlay" id="clientModalOverlay">
      <div class="modal">
        ...
        <div class="modal-footer">
          <button class="btn btn-primary" style="width:auto" id="clientModalSave">Fermer</button>
        </div>
      </div>
    </div>
  `;
  document.getElementById('clientModalSave').onclick = async ()=>{
    ...
    const hasContent = firstName || lastName
      || document.getElementById('cmPhone').value.trim()
      || document.getElementById('cmEmail').value.trim()
      || document.getElementById('cmAddress').value.trim()
      || document.getElementById('cmComment').value.trim();
    if(!c.manualId && !hasContent){ closeModal(); return; }
