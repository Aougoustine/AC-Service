...
(async function(){
  ...
  if(editReportId && state.interventions.some(iv=>iv.id===editReportId)){
    ...
    // Dépôt créé sans RDV lié (création libre) : pas de fiche à pré-remplir depuis un
    // RDV, on ouvre un rapport vierge — pense à reporter le numéro de dossier
    // manuellement si tu veux qu'il soit regroupé avec ce dépôt dans Dossiers.
    openIntervModal();
    history.replaceState(null, '', 'planning.html');
  }
})();
...
function confirmedForDate(dateStr){
  return state.appointments.filter(a=>a.status==='Confirmé' && a.date===dateStr && a.requestType!=='depot');
}

// Vue du jour utilisée par renderDayDetail : pour aujourd'hui/futur, uniquement les
// RDV "Confirmé" (organisation de tournée). Pour une date passée, tous les statuts
// sauf "Nouveau" (utile pour retrouver un RDV déjà traité/annulé sans repasser par Dossiers).
function dayViewAppointments(dateStr){
  if(dateStr < todayISO()){
    return state.appointments.filter(a=>a.date===dateStr && a.status!=='Nouveau' && a.requestType!=='depot');
  }
  return confirmedForDate(dateStr);
}

/* ═══════════ Vue d'ensemble de la page (demandes en attente + bande de jours + rapports) ═══════════ */
function renderPlanning(){
  ...
  main.innerHTML = `
    ...
    <div class="section-block">
      ...
      <div class="filters">
        <input id="rdvPendingSearch" placeholder="Rechercher un client...">
      </div>
      <div id="rdvPendingList"></div>
    </div>
    ...
    <div class="section-block">
      <p style="font-size:.85rem;color:var(--text-light);margin-bottom:1rem;">
        Les rendez-vous validés apparaissent ici automatiquement (ceux venant du site y arrivent directement). Naviguez mois par mois, ou revenez à aujourd'hui en un clic.
      </p>
  ...
  document.getElementById('importBtn').onclick = ()=>importFromGateway(state, ()=>{ renderNavbar('planning', state); renderPlanGrid(); renderDayDetail(planSelectedDate); drawRdvPendingList(); });
  document.getElementById('rdvPendingSearch').oninput = drawRdvPendingList;
...
function applyPlanRefresh(newState){
  const prevPendingCount = pendingRdvList().length;
  const prevDevisStatus = {};
  (state.devis||[]).forEach(d=>{ prevDevisStatus[d.id] = d.status; });

  state = newState;

  const newPendingCount = pendingRdvList().length;
  if(newPendingCount > prevPendingCount){
    showToast('📥 '+(newPendingCount-prevPendingCount)+' nouveau(x) rendez-vous reçu(s).');
  }
...
function pendingRdvList(){
  return state.appointments.filter(a=>!['Confirmé','Terminé','Annulé'].includes(a.status) && a.requestType!=='depot');
}

const PENDING_REASONS = ["", "⏳ Devis en attente", "🔩 Commande pièce", "🔁 À replanifier"];

function drawRdvPendingList(){
  const search = (document.getElementById('rdvPendingSearch').value||'').toLowerCase();
  let list = pendingRdvList().sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
  if(search) list = list.filter(a=>(a.clientName||'').toLowerCase().includes(search));
  const el = document.getElementById('rdvPendingList');
  if(!list.length){
    el.innerHTML = `<div class="empty-state"><div class="mark">📥</div>Aucune demande en attente. Cliquez sur "+ Nouveau RDV" pour en ajouter une.</div>`;
    return;
  }
  el.innerHTML = list.map(a=>{
    const line1 = `${a.dossierNumber?`<span style="color:var(--cyan);">${escapeHtml(a.dossierNumber)}</span> — `:''}<strong>${escapeHtml(a.clientName)}</strong>`;
    const addressLine = a.address
      ? `<div style="display:flex;gap:.4rem;align-items:flex-start;"><span>📍</span><a href="${wazeAppUrl(fullAddress(a))}" onclick="event.stopPropagation()" style="color:var(--cyan);word-break:break-word;">${escapeHtml(fullAddress(a))}</a></div>`
      : `<div style="display:flex;gap:.4rem;align-items:flex-start;"><span>📍</span><span>—</span></div>`;
    const phoneLine = a.phone ? `<div style="display:flex;gap:.4rem;align-items:flex-start;"><span>📞</span><span style="word-break:break-word;">${escapeHtml(a.phone)}</span></div>` : '';
    const emailLine = a.email ? `<div style="display:flex;gap:.4rem;align-items:flex-start;"><span>✉️</span><span style="word-break:break-word;">${escapeHtml(a.email)}</span></div>` : '';
    return `
    <div class="item-row">
      <div class="item-main" style="cursor:pointer;" onclick="openRdvDetail('${a.id}')">
        <div class="title" style="margin-bottom:.4rem;">${line1}</div>
        <div class="meta" style="display:flex;flex-direction:column;gap:.3rem;">
          ${addressLine}
          ${phoneLine}
          ${emailLine}
        </div>
      ...
      <div class="item-actions">
        <select class="pendingReasonSelect badge ${statusBadgeClass(a.status)}" data-id="${a.id}" onclick="event.stopPropagation()">
          ${PENDING_REASONS.map(r=>`<option value="${escapeHtml(r)}" ${(a.pendingReason||'')===r?'selected':''}>${r?escapeHtml(r):escapeHtml(a.status)}</option>`).join('')}
        </select>
        <button class="btn btn-ghost btn-small" onclick="validateRdv('${a.id}')">✅ Valider</button>
        ${(()=>{ const existingDevis = (state.devis||[]).find(d=>d.linkedRdvId===a.id); return existingDevis
          ? `<button class="btn btn-ghost btn-small" onclick="openDevisModal('${existingDevis.id}')">📄 Modifier le devis</button>`
          : `<button class="btn btn-ghost btn-small" onclick="openDevisModal(null, '${a.id}')">📄 Créer un devis</button>`; })()}
        <button class="btn btn-danger btn-small" onclick="deleteRdv('${a.id}')">Supprimer</button>
      </div>
    </div>
  `;}).join('');
  document.querySelectorAll('.pendingReasonSelect').forEach(sel=>{
    sel.onchange = ()=>setPendingReason(sel.dataset.id, sel.value);
  });
}

async function setPendingReason(id, reason){
  const a = state.appointments.find(x=>x.id===id);
  if(!a) return;
  const backup = a.pendingReason;
  a.pendingReason = reason;
  const ok = await saveState(state);
  if(!ok){ a.pendingReason = backup; drawRdvPendingList(); return; }
  showToast("Raison de l'attente mise à jour.");
}

async function validateRdv(id){
  const a = state.appointments.find(x=>x.id===id);
  if(!a) return;
  ...
  renderPlanGrid();
  renderDayDetail(planSelectedDate);
  showToast("Rendez-vous validé — il apparaît maintenant dans le calendrier.");
}
    ...
    <div class="modal-overlay" id="rdvDetailOverlay">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          ${a.status==='Annulé' ? `<button class="btn btn-ghost" id="rdvDetailRedoDevis">🔁 Remettre en attente + refaire le devis</button>` : ''}
...
function openRdvModal(id){
  ...
  const a = editing || {id:uid(), firstName:'',lastName:'',clientName:'',phone:'',email:'',address:'',postalCode:'',city:'',date:todayISO(),time:'Matin',timeNote:'',applianceType:state.settings.applianceTypes[0]||'',brand:'',model:'',issue:'',status:'Nouveau',source:'Manuel',referral:''};
    ...
    <div class="modal-overlay" id="rdvOverlay">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" style="width:auto" id="mSave">Enregistrer</button>
        </div>
      </div>
    </div>
  `;
  ...
  // Le clic en dehors du formulaire ne le ferme plus (évite de perdre une saisie
  // par erreur) — seuls les boutons "Annuler" et "Enregistrer" permettent de quitter.
...
// Remet un RDV annulé en attente ET réinitialise le devis lié (même numéro, mais
...
async function redoRdvAndDevis(id){
  ...
  if(linkedDevis){
    showToast("Rendez-vous remis en attente — devis réinitialisé, modifiez-le puis renvoyez-le.");
    openDevisModal(linkedDevis.id);
  } else {
    showToast("Rendez-vous remis en attente — aucun devis existant à refaire, vous pouvez en créer un nouveau.");
  }
}
...
async function replanifyRdv(id){
  ...
  renderNavbar('planning', state);
  drawRdvPendingList();
  showToast("Rendez-vous remis en attente — la place a été libérée sur le site.");
...
const RDV_FOLLOWUP_OPTIONS = ["À faire", "Client injoignable", "Rendez-vous annulé", "Dossier assurance en cours", "Retour SAV fabricant", "Terminé", "Commande pièce", "Devis refusé"];
...
function renderDayDetail(dateStr){
  ...
  document.querySelectorAll('.followUpSelect').forEach(sel=>{
    sel.onchange = ()=>setFollowUp(sel.dataset.id, sel.value);
  });
}
    ...
    <div style="margin-bottom:1.3rem;">
      ...
      ${items.map((a,idx)=>{
        ...
        const reportBtn = existingReport
          ? `<button class="btn btn-ghost btn-small" onclick="openIntervModal('${existingReport.id}')">✏️ Modifier le rapport</button>`
          : `<button class="btn btn-ghost btn-small" onclick="openIntervModal(null, '${a.id}')">📝 Créer le rapport</button>`;
        const isConfirme = a.status === 'Confirmé';
        return `
        <div class="item-row">
          <div class="item-main">
            <div class="title">${a.dossierNumber?`<span style="color:var(--cyan);">${escapeHtml(a.dossierNumber)}</span> — `:''}${escapeHtml(a.clientName)} · ${escapeHtml(a.applianceType||'')} ${!isConfirme?`<span class="badge ${statusBadgeClass(a.status)}">${escapeHtml(a.status)}</span>`:''}</div>
            <div class="meta">
              ${a.address ? `<a href="${wazeAppUrl(fullAddress(a))}" style="color:var(--cyan);">📍 ${escapeHtml(fullAddress(a))}</a>` : '—'}
              ${a.phone ? ` · <a href="tel:${escapeHtml(a.phone)}" style="color:var(--cyan);text-decoration:none;">📞 ${escapeHtml(a.phone)}</a>` : ''}
              ${(a.brand||a.model) ? ' · '+escapeHtml([a.brand,a.model].filter(Boolean).join(' ')) : ''}<br>
              ${escapeHtml(a.issue||'')}
              ${a.timeNote ? `<br>🕒 <em>${escapeHtml(a.timeNote)}</em>` : ''}
            </div>
          </div>
          <div class="item-actions" style="align-items:center;">
            ...
            ` : ''}
            ${reportBtn}
            <select class="followUpSelect" data-id="${a.id}">
              ${RDV_FOLLOWUP_OPTIONS.map(o=>`<option value="${escapeHtml(o)}" ${(a.followUp||'À faire')===o?'selected':''}>${escapeHtml(o)}</option>`).join('')}
            </select>
            ${isConfirme ? `<button class="btn btn-danger btn-small" onclick="cancelPlanItem('${a.id}')">❌ Annuler</button>` : ''}
          </div>
        </div>
      `;}).join('')}
    </div>
...
async function setFollowUp(id, value){
  const a = state.appointments.find(x=>x.id===id);
  if(!a) return;
  const backup = a.followUp;
  a.followUp = value;
  const ok = await saveState(state);
  if(!ok){ a.followUp = backup; renderDayDetail(planSelectedDate); return; }
  showToast("Suivi mis à jour.");
}

// Retire le RDV du calendrier (il n'apparaîtra plus, puisque seul le statut "Confirmé"
// y est affiché) SANS le supprimer : il part dans Historique (statut "Annulé"), d'où
// il pourra être replanifié ou définitivement supprimé.
// Retire le RDV du calendrier et le renvoie directement dans "Demandes en attente
// de validation" (statut "Nouveau") — plus de passage par Dossiers pour un RDV annulé.
// La place réservée côté site est libérée immédiatement. Depuis les demandes en
// attente, vous pourrez modifier la date/le créneau puis revalider, ou le supprimer
// définitivement.
async function cancelPlanItem(id){
  const a = state.appointments.find(x=>x.id===id);
  if(!a) return;
  if(!confirm("Annuler ce rendez-vous ? Il sera retiré du calendrier et renvoyé dans \"Demandes en attente de validation\", où vous pourrez changer sa date ou le supprimer définitivement.")) return;
  ...
  renderDayDetail(planSelectedDate);
  drawRdvPendingList();
  showToast("Rendez-vous annulé — renvoyé dans les demandes en attente, la place est de nouveau disponible sur le site.");
}
...
function newDevice(){
  return { id:uid(), applianceType:state.settings.applianceTypes[0]||'', brand:'', model:'', serialNumber:'', clientSymptom:'', diagnosis:'', workDone:'', result:PASSAGE_TYPES[0], insuranceComment:'', forfait:'', amount:'', partsUsed:[], partsNeeded:'', photos:[], plateePhoto:null, plateePhotoSource:null };
}
...
function openDevisModal(id, preLinkRdvId){
  ...
  if(editing){
    ...
    if(!devis.items || !devis.items.length) devis.items = [newDevisItem()];
    ...
    devis = {
      ...
      items: [newDevisItem(preRdv ? preRdv.applianceType : null)],
      discount: 0,
      comment: '',
      status: 'Brouillon',
      publicToken: null,
    ...
    if(preLinkRdvId){
      ...
      if(rdv){
        ...
        devis.city = rdv.city || '';
        if(rdv.applianceType || rdv.brand || rdv.model){
  ...
  modalRoot.innerHTML = `
    <div class="modal-overlay" id="devisOverlay">
      <div class="modal">
        ...
        ${!readOnly ? `<button type="button" class="btn btn-ghost btn-small" id="addDevisItemBtn" style="margin-top:.6rem;">+ Ajouter une ligne</button>` : ''}
        ...
        <div class="modal-footer" style="flex-wrap:wrap;">
          <button class="btn btn-ghost" id="devCancel">Fermer</button>
          ...
          ${!readOnly ? `<button class="btn btn-primary" style="width:auto" id="devSaveBtn">Enregistrer</button>` : ''}
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('devCancel').onclick = closeModal;
  ...
  if(!readOnly){
    document.getElementById('devSaveBtn').onclick = async ()=>{
      if(!document.getElementById('devFirstName').value.trim() && !document.getElementById('devLastName').value.trim()){
        showToast("Le prénom ou le nom du client est requis.");
        return;
      }
...
function openIntervModal(id, preLinkRdvId, returnToDossiers){
  ...
  if(editing){
    ...
    if(!i.devices || !i.devices.length){
      i.devices = [{
        ...
        diagnosis: i.diagnosis||'', workDone: i.workDone||'', result: i.passageType || PASSAGE_TYPES[0],
    ...
    i = {
      ...
      status:'Brouillon', timeSpent:'', amount:'', discount:'', observations:'', devices:[newDevice()],
      signature:'', linkedRdvId:null, paymentReceived:false
    };
  }
  ...
  modalRoot.innerHTML = `
    <div class="modal-overlay" id="intervOverlay">
      <div class="modal">
        ...
        <div class="field"><label>Observations supplémentaires</label><textarea id="iObservations" placeholder="Ex : client absent au 1er passage, chien dans le jardin…">${escapeHtml(i.observations||'')}</textarea></div>

        <div class="modal-row">
          <div class="field"><label>Temps passé (min)</label><input type="number" id="iTime" value="${i.timeSpent||''}"></div>
          <div class="field"><label>Remise (€)</label><input type="number" step="0.01" min="0" id="iDiscount" value="${i.discount||''}" placeholder="0"></div>
        </div>
        <div class="field"><label>Montant total (après remise)</label><div style="padding:.7rem .8rem;font-family:'Rajdhani',sans-serif;font-weight:700;font-size:1.1rem;color:var(--cyan);" id="iTotalDisplay">${money(i.amount||0)}</div></div>
        <div class="field">
          <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;">
            <input type="checkbox" id="iPaymentReceived" ${i.paymentReceived?'checked':''} style="width:auto;">
            Paiement effectué
          </label>
        </div>
        ...
        <div class="modal-footer" style="flex-wrap:wrap;">
          ...
          <button class="btn btn-ghost" id="iCancel">Fermer</button>
          <button class="btn btn-primary" style="width:auto" id="iSave">Enregistrer</button>
        </div>
      </div>
    </div>
  `;
  ...
  function deviceCardHtml(d, idx){
    const isDepotDossier = !!(i.dossierNumber && state.deposits.some(dep=>dep.dossierNumber===i.dossierNumber));
    const resultOptions = isDepotDossier ? ["Réparé","Irréparable"] : PASSAGE_TYPES;
    const displayResult = isDepotDossier
      ? (d.result==='Irréparable' ? 'Irréparable' : 'Réparé')
      : d.result;
    return `
      <div class="device-card" data-device-index="${idx}" style="border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:1rem;">
        ...
        <div class="field"><label>Résultat de l'intervention</label>
          <select class="devResult" data-idx="${idx}">${resultOptions.map(p=>`<option ${p===displayResult?'selected':''}>${p}</option>`).join('')}</select>
        </div>
        <div class="field devInsuranceField" data-idx="${idx}" style="display:${d.result==='Irréparable'?'block':'none'};">
        ...
        <div class="field"><label>Pièces utilisées (déduites du stock)</label>
          <div class="parts-picker">
            ${!state.stock.length ? '<div style="color:var(--text-light);font-size:.82rem;">Aucune pièce en stock.</div>' : state.stock.map(s=>{
              const used = (d.partsUsed||[]).find(p=>p.stockId===s.id);
              return `<div class="part-line"><span>${escapeHtml(s.name)} <span style="color:var(--text-light)">(dispo: ${s.qty})</span></span><input type="number" min="0" class="devPartQty" data-idx="${idx}" data-stockid="${s.id}" value="${used?used.qty:0}"></div>`;
            }).join('')}
          </div>
        </div>
        <div class="field">
          <label>Besoins en pièces à commander <span style="font-weight:400;font-size:.8em;color:var(--text-light);">(hors stock — nom, référence, quantité...)</span></label>
        ...
        <div class="field"><label>Montant (€)</label><input type="number" step="0.01" class="devAmount" data-idx="${idx}" value="${d.amount||''}"></div>
  ...
  function updateTotalDisplay(){
    ...
    const discountInput = document.getElementById('iDiscount');
  ...
  function syncDevicesFromDOM(){
    document.querySelectorAll('.device-card').forEach(card=>{
      ...
      d.result = card.querySelector('.devResult').value;
      const insEl = card.querySelector('.devInsuranceComment');
      d.insuranceComment = insEl ? insEl.value.trim() : (d.insuranceComment||'');
      d.amount = card.querySelector('.devAmount').value;
      // Quantités "voulues" pour cet appareil — pas encore répercutées sur le stock réel,
      // ça n'arrive qu'une seule fois, à l'enregistrement final (voir readFormIntoI).
      const parts = [];
      card.querySelectorAll('.devPartQty').forEach(inp=>{
        const qty = Number(inp.value)||0;
        if(qty>0){
          const stockId = inp.dataset.stockid;
          const s = state.stock.find(x=>x.id===stockId);
          parts.push({stockId, name:s?s.name:'', qty});
        }
      });
      d.partsUsed = parts;
  ...
  document.getElementById('devicesContainer').addEventListener('change', (e)=>{
    if(e.target.classList.contains('devResult')){
      const idx = e.target.dataset.idx;
      const field = document.querySelector('.devInsuranceField[data-idx="'+idx+'"]');
      if(field) field.style.display = e.target.value === 'Irréparable' ? 'block' : 'none';
    }
  });
  document.getElementById('devicesContainer').addEventListener('input', (e)=>{
    if(e.target.classList.contains('devAmount')) updateTotalDisplay();
  });
  document.getElementById('iDiscount').addEventListener('input', updateTotalDisplay);
  ...
  // Utilisé à la fois par "Enregistrer", "Fermer" (brouillon) et les boutons PDF,
  ...
  function readFormIntoI(){
    ...
    // Restitue le stock de TOUS les appareils précédemment enregistrés avant de
    // recalculer les déductions à partir de ce qui est actuellement à l'écran.
    (i.devices||[]).forEach(d=>{
    ...
    i.timeSpent = document.getElementById('iTime').value;
    i.observations = document.getElementById('iObservations').value.trim();

    // Synchronise l'appareil actuellement affiché (les autres sont déjà à jour dans
    // i.devices depuis les précédents changements de sélection), puis on traite TOUS
    // les appareils — la déduction de stock ne doit avoir lieu qu'une fois ici, pour
    // chacun d'eux, pas seulement celui actuellement visible à l'écran.
    syncDevicesFromDOM();
    (i.devices||[]).forEach(d=>{
      const wantedParts = d.partsUsed || [];
      const deducted = [];
      wantedParts.forEach(p=>{
        const qty = Number(p.qty)||0;
        if(qty>0){
          const s = state.stock.find(x=>x.id===p.stockId);
          deducted.push({stockId:p.stockId, name:s?s.name:p.name, qty});
          if(s) s.qty = Math.max(0, Number(s.qty) - qty);
        }
      });
      d.partsUsed = deducted;
      d.photos = (d.photos||[]).filter(p=>!p.uploading).map(p=>({ id: p.id, name: p.name }));
    });

    i.discount = Number(document.getElementById('iDiscount').value) || 0;
    i.paymentReceived = document.getElementById('iPaymentReceived').checked;
    const subtotal = i.devices.reduce((s,d)=>s+(Number(d.amount)||0), 0);
    i.amount = Math.max(0, subtotal - i.discount);
    delete i.photos; // champ obsolète (les photos vivent maintenant par appareil)
    return stockBackup;
  }

  async function persist(forcedStatus){
    const stockBackup = readFormIntoI();
  ...
  document.getElementById('iSave').onclick = async ()=>{
    ...
    drawIntervList();
    showToast("Rapport d'intervention enregistré.");
  };

  // "Fermer" : ferme la fenêtre SANS rien perdre. Si des informations ont été saisies,
  // elles sont enregistrées en brouillon (nouveau rapport jamais sauvegardé) plutôt que
  // d'être jetées ; sinon (formulaire resté vide) on ferme simplement, sans créer de
  // rapport vide, et on nettoie les photos éventuellement déjà envoyées.
  ...
  document.getElementById('iCancel').onclick = async ()=>{
    const hasContent = document.getElementById('iFirstName').value.trim()
      || document.getElementById('iLastName').value.trim()
    ...
    const ok = await persist(!editing && !savedThisSession ? 'Brouillon' : undefined);
    closeModal();
    if(returnToDossiers){ window.location.href = 'dossiers.html'; return; }
    if(ok){
      drawIntervList();
      showToast("Rapport enregistré en brouillon.");
    }
  };

  // Le clic en dehors du rapport ne le ferme plus (évite de perdre une saisie par erreur) —
  // seuls les boutons "Fermer" et "Enregistrer" permettent de quitter la fenêtre.
}
